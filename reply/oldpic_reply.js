 ...
 */
export async function sendMessageOld(replyToken, client, formattedTransactionDateTime, transRef, amount, fromName, fromBank, fromAccount, toName, toBank ,toAccount) {
  const flexMessage = {
    "type": "bubble",
    "hero": {
      "type": "box",
  ...
  try {
    await client.replyMessage(replyToken, { type: "flex", altText: "🟡 สลิปย้อนหลังเกินกำหนด ⚠", contents: flexMessage });
    console.log("ตอบกลับแล้ว สลิปย้อนหลังเกินกำหนด ⚠");
    broadcastLog("ตอบกลับแล้ว สลิปย้อนหลังเกินกำหนด ⚠"); // ส่งข้อความไปยังฟังก์ชัน broadcastLog
  } catch (err) {
